...
def latest_products(context):
    ...
    if user.is_authenticated:
        wishlist_items = list(WishListModels.objects.filter(user=user).values_list('product__id', flat=True))
...
def similar_products(context, product):
    ...
    base_queryset = ProductModels.objects.filter(
        status=ProductStatusModels.publish.value,
        category__in=products_category
    ...
    if len(similar_products) < 4:
        ...
        extra_products = ProductModels.objects.filter(
            status=ProductStatusModels.publish.value
        ).exclude(id__in=[p.id for p in similar_products] + [product.id]).order_by('-created_date')[:needed]
    ...
    if user and user.is_authenticated:
        wishlist_items = list(
            WishListModels.objects.filter(user=user)
