...
class ZarinpalVerifyView(View):
    def get(self, request):
        ...
        if not authority:
            return render(request, "payment/failed.html", {"message": "اطلاعات پرداخت ناقص است."})
        ...
        if status != "OK":
            payment.status = 2
            payment.save()
            return render(request, "order/failed.html", {"message": "پرداخت توسط کاربر لغو شد."})
        ...
        if result.get("Status") == 100:
            ...
            payment.status = 2
            payment.save()
            return render(request, "payment/complete.html", {"message": "پرداخت تایید نشد."})
# ======================================================================================================================